...
SELECT id FROM orders WHERE customer_id IN (SELECT id FROM customers WHERE fname = 'Jane' AND lname = 'Doe');
...
SELECT name, SUM(num_cupcakes) AS sum FROM cupcakes LEFT JOIN orders ON cupcakes.id = orders.cupcake_id GROUP BY name ORDER BY name ASC;